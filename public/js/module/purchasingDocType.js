 ...
 */
var saveOrUpdatePurchasing = function () {

    ////entrada de datos
    var form = $('#form1');

    jQuery.ajax({
        url: PATHAPP + 'catalogs/PurchasingDocumentTypeSave',
        type: 'POST',
        dataType: 'json',
        data: form.serialize(),
        beforeSend:  function () {
        ...
        success: function (response) {
            ...
            if(response.action=='new'){
                location.replace(PATHAPP+'catalogs/PurchasingDocumentTypeList')
            }else{
...
var deletepayPurDoc = function (id) {
    $.confirm({
        text: "Esta seguro que desea borrar este registro?",
        title: "Borrar Tipo de pago",
        confirm: function () {
            jQuery.ajax({
                url: PATHAPP + 'catalogs/PurchasingDocumentTypeDel',
                type: 'POST',
                data: {"id": id},
                success: function (response) {
                    location.reload();
