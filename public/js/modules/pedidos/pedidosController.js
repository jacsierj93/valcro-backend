...
MyApp.controller('PedidosCtrll', function ($scope,$mdSidenav,$timeout
    ...
    $scope.openTempDoc = function(doc){
        ...
        if (segurity('editPedido')) {
            ...
            $scope.navCtrl.value="detalleDoc";
            $scope.navCtrl.estado=true;
            $scope.Docsession.global=(!doc.uid) ? 'upd' : 'temp';
            $scope.Docsession.block = (!doc.uid) ? true : false;
        }
    ...
    $scope.saveFinal = function(){
        if($scope.Docsession.global == 'new' || $scope.Docsession.global == 'temp'){
        ...
        }else if($scope.Docsession.global == 'upd'){
            ...
            console.log("$scope.Docsession.global", $scope.Docsession.global);
        }
    ...
    $scope.$watch("FormHeadDocument.$pristine", function(newVal,oldVal){
        if(!newVal && $scope.module.layer ==  'detalleDoc' ){
        if(timeSave){
...
MyApp.service('setGetOrder', function(DateParse, Order, providers, $q) {
    ...
    return {
        ...
        change:function(form,fiel, value){
            ...
            console.log("cambaido por", form );
            console.log("cambaido por fie", fiel );
            console.log("cambaido por fiel", value );
        },
    ...
    return function (data){
        ...
        angular.forEach(data.to, function(v,k){
            $timeout(function(){
                ...
                console.log("clicker "+ index, clicker);
                clicker[0].click();
